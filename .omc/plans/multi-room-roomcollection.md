...
## CapturedStructure splitter adapter (planner 2026-06-17)

**Goal:** ingest a REAL Apple RoomPlan `CapturedStructure` export (now in repo: `tests/fixtures/roomplan_real/capturedstructure_{multiroom,single}.json`, MIT) → N single-room `RoomModel` → feed the EXISTING ADR 0049 `RoomCollection` composition layer (shipped v0.40–0.42). This is the input-path half of ADR 0047's re-open. **NEW ADR: `docs/adr/0050-roomplan-capturedstructure-splitter.md` (Status: Proposed).**

**Baseline:** v0.42.0 — default 666p/7s · web 86p/3s · ruff·mypy clean. Hard rules: NO FAKE NUMBERS / NO FAKE CAPABILITY, additive-only (single-room + existing `collection` path byte-equal), full gate each phase.

### Schema facts (independently re-verified by `json.load`, NOT assumed)
- Top-level: `coreModel`(base64 proprietary), `referenceOriginTransform[16]`, `version`(2), `story`, `sections[]`, flat `walls/doors/windows/openings/objects/floors[]`.
- `Section` = `{label, story, center[x,y,z]}` ONLY (no geometry). multiroom labels: bedroom, bedroom, bathroom, **unidentified**. **NO element→room FK anywhere.**
- `Surface`: `category`/`confidence` = single-key dict (`{"wall":{}}`, `{"door":{"isOpen":false}}`); `dimensions[w,h,0]`; `transform`=**flat 16-float column-major** simd_float4x4; `polygonCorners`(wall=`[]`, floor=4 corners); UUID `identifier`/`parentIdentifier`.
- **CRITICAL #1: `floors[]` has exactly 1 entry** (building-wide 6.43×7.43 m) for ALL 4 sections → per-room floor polygons DO NOT EXIST in the export.
- **CRITICAL #2 (verified):** `np.asarray(flat,float).reshape(4,4).T` → col3=origin `[4.055,-0.265,-4.379]`, col0=unit width-dir `[0.029,0,1.0]` (norm 1.0) → **exactly matches** existing `roomplan.py::_wall_polygon_from_transform` contract. Helper reusable after this reshape.
- NO material_hint, NO ceilings, NO room dims.

### Resolved design decisions
- **D-B Parsing:** `_mat4_from_flat = reshape(4,4).T`; enum-dict via `next(iter(d))`; meters/Y-up (no conversion). REUSE existing `_wall_polygon_from_transform`/`_polygon_3d`/`_project_to_floor_polygon`/`_material_for_hint`/object-kind logic by import. NEW module `roomestim/adapters/roomplan_structure.py`; existing `roomplan.py` NOT edited.
- **D-C Room assignment (load-bearing honesty):** PRIMARY = floor-plane (x,z) nearest-section-center, story-matched (section.center y is a constant structure-mean → no vertical signal). doors/windows follow `parentIdentifier`→parent wall's room and **re-base `wall_index`** in that room's walls-only frame (ADR 0037); parent unmatched → nearest-center + `wall_index=None`. Sections **index-addressed** (two same-label bedrooms kept distinct → name suffix `-2`). `unidentified` kept as its own room (never merged). Tie-break = lowest section index. Split is RECONSTRUCTED → `_disclosure.py::ROOMPLAN_STRUCTURE_SPLIT_NOTE` (new single-source string).
- **D-D Per-room RoomModel:** floor_polygon = **convex hull of assigned walls' floor-projected endpoints** (NOT `floors[]` — building-wide single poly; Voronoi split REJECTED = fabricated boundary). ceiling_height_m = **median of assigned wall heights** (`ceiling_coverage=None`, `ceiling_confidence="unknown"`). No ceiling surface. objects via enum-dict→existing kind map (sofa/table/bed/storage kept; chair/sink/toilet ignored). listener_area = existing `default_listener_area`. provenance=`"measured"` (LiDAR geometry); membership-heuristic carried by disclosure+warning, NOT by overloading the literal.
- **D-E CLI:** NEW subcommand `roomestim structure --in-structure PATH.json [placement flags] --name N --out-dir D` → `_cmd_structure` mirrors `_cmd_collection` (parse_structure → N RoomModel → `_run_placement` per room → `write_room_yaml`/`write_layout_yaml` → `RoomCollection` → `write_collection_yaml` + optional combined glTF/USD reuse). Existing `collection`/`ingest` subcommands NOT touched (contract clarity).
- **D-validation (no GT):** structural/sanity ONLY — fixture has no independent GT. Partition invariant (every wall assigned exactly once; Σ == total walls); each room footprint finite/>0/within building bbox, ceiling∈[2.0,3.5]; degenerate section (<3 walls) → low-confidence + warning; per-room layout byte-identical to standalone `place`; provenance + split disclosure present. NO accuracy assertions.

### Phase S1 — Thin vertical slice (parse → split → N RoomModels → RoomCollection → manifest, walls+floor only)
**Scope:** new `adapters/roomplan_structure.py` (`parse_structure`, `_mat4_from_flat`, enum-dict parser, nearest-section-center wall assignment, per-room wall-hull footprint, median-wall-height ceiling, N `RoomModel`); new `_disclosure.py::ROOMPLAN_STRUCTURE_SPLIT_NOTE`; new `structure` CLI subcommand wiring to existing `RoomCollection` + `write_collection_yaml` + per-room placement. Objects/doors/windows/openings DEFERRED to S2.
**Files ADDED:** `roomestim/adapters/roomplan_structure.py`; `tests/test_roomplan_structure_split.py`; `tests/test_structure_cli.py`.
**Files TOUCHED:** `roomestim/reconstruct/_disclosure.py` (append new constant + `__all__` — additive); `roomestim/cli.py` (new `_add_structure_parser` + `_cmd_structure` + 1 dispatch branch + 1 registration — existing subcommands byte-unchanged); `roomestim/__init__.py` (optional additive export). **Single-room code TOUCHED: ZERO** (`roomplan.py`, `model.py`, existing adapters/exports/placement/`collection.py` untouched).
**Acceptance:** multiroom fixture → 4 `RoomModel` (incl. `unidentified`); partition invariant (Σ walls == 20); each footprint finite/>0/within 6.43×7.43 bbox; ceiling∈[2.0,3.5]; `roomestim structure --in-structure …multiroom.json --algorithm vbap --out-dir D` writes 4×`room.*.yaml` + 4×`layout.*.yaml` + `collection.yaml`; **each per-room layout byte-identical to standalone `place` on that room.yaml**; single fixture → 1 room; ALL existing single-room/`collection` goldens byte-unchanged.
**Gate:** `/home/seung/miniforge3/bin/python -m pytest -q` + `-m web` + ruff + mypy — GREEN vs v0.42.0 (666p/7s · 86p/3s · clean); new tests additive.

### Phase S2 — Objects + doors/windows + combined export reuse
**Scope:** assign objects (enum-dict → existing kind map) to rooms; doors/windows via `parentIdentifier`→parent-wall room with **re-based per-room `wall_index`** (fallback nearest-center, `wall_index=None`); wire optional `--combined-gltf`/`--combined-usd` (reuse ADR 0049 Phase 2/3 writers). openings still deferred.
**Files ADDED:** tests for object/door/window assignment + wall_index re-basing + combined export.
**Files TOUCHED:** `adapters/roomplan_structure.py` (extend); `cli.py` (`_cmd_structure` passes combined flags — additive). Single-room code: ZERO.
**Acceptance:** 13 objects assigned (sofa/table/bed/storage kept, chair/sink/toilet dropped per existing policy); each door/window's `wall_index` in-range for ITS room (no cross-room index, ADR 0037 guard passes); combined glTF/USD opens; goldens byte-equal.
**Gate:** full gate GREEN.

### Phase S3 — Robustness + openings + disclosure hardening + README
**Scope:** degenerate-section handling (<3 walls → low-confidence footprint + `UserWarning`), equidistant tie-break test, `unidentified`-section explicit handling test, `openings[]` ingest (same path as walls), finalize disclosure wiring (room.yaml sidecar + CLI note + README honest section). Document known failure modes (nested room, same-label adjacency).
**Files ADDED:** robustness tests; README section.
**Files TOUCHED:** `adapters/roomplan_structure.py`; `cli.py` notice; README/CHANGELOG. Single-room code: ZERO.
**Acceptance:** degenerate section emits warning not crash; failure modes documented; disclosure single-source referenced everywhere; goldens byte-equal.
**Gate:** full gate GREEN. Then version bump (MINOR/additive) + ADR 0050 → Accepted + memory update.

### Top risk to flag BEFORE coding
**The room-assignment heuristic can mis-partition nested / adjacent / same-label rooms and there is NO ground truth to measure how wrong it is.** Verified: nearest-section-center already leaves one of the two same-label `bedroom` sections with 0 walls in the multiroom fixture, and a bathroom-inside-bedroom would be swallowed by the larger room. This is the load-bearing honesty risk — the per-room split is a reconstruction, not Apple data. Mitigation is honesty, not accuracy: single-source disclosure, `UserWarning` on degenerate sections, a partition-invariant test (every wall assigned exactly once), and a hard NO on any per-room accuracy claim or aggregate volume/RT60. Secondary risk: per-room footprint is a wall-hull estimate (concave over-estimation) — never present it as a measured floor polygon.

### RESUME POINTER (CapturedStructure splitter)
- [x] planner: design + ADR 0050 (Proposed) — DONE 2026-06-17 (this section + `docs/adr/0050-roomplan-capturedstructure-splitter.md`).
- [x] **executor: Phase S1 (thin vertical slice) — DONE 2026-06-17.**
  - Files ADDED: `roomestim/adapters/roomplan_structure.py` (`parse_structure`, `_mat4_from_flat`, `_enum_key`, `_assign_walls_to_sections`, `_section_footprint`, `_unique_name`, `_room_from_section`); `tests/test_roomplan_structure_split.py` (8 tests); `tests/test_structure_cli.py` (3 tests).
  - Files TOUCHED (additive only): `roomestim/reconstruct/_disclosure.py` (+`ROOMPLAN_STRUCTURE_SPLIT_NOTE` + `__all__`); `roomestim/cli.py` (+`_add_structure_parser` +`_cmd_structure` +1 registration +1 dispatch branch — existing subcommands byte-unchanged); `roomestim/__init__.py` (+`parse_structure` export). **Single-room code (`roomplan.py`/`model.py`/`collection.py`/existing adapters/exporters/readers/placement): ZERO touch.**
  - **Actual wall partition on `capturedstructure_multiroom.json` (20 walls, nearest-section-center heuristic, story-matched):** `bedroom`=5, `bedroom-2`=6, `bathroom`=3, `unidentified`=6 → Σ=20 (partition invariant holds). **NO degenerate section** in this fixture (bathroom=3 is the minimum, still ≥3) — this CONTRADICTS the planner's pre-verification claim that "one of the two same-label bedrooms gets 0 walls." Degenerate handling (<3 distinct floor points → minimal box, no crash) is implemented + unit-tested via a synthetic far-away section (0 walls), but is NOT exercised by the real fixture.
  - Per-room footprint = floor-projected convex hull of assigned walls (over-estimate); ceiling_height_m = median assigned wall height = 2.441 m for all 4 rooms (∈[2.0,3.5]); provenance="measured"; objects=[] (S2). Membership-heuristic honesty via `ROOMPLAN_STRUCTURE_SPLIT_NOTE` (printed to stderr by `_cmd_structure`) + disclosure constant; NOT via overloading provenance.
  - DEVIATION (honest): dropped the planned "footprint within building bbox" sanity assertion — wall positions are in the WORLD frame while the export's single `floors[]` polygon is in the FLOOR's LOCAL transform frame, so a shared-bbox bound would be fabricated. Replaced with a finiteness (no NaN/inf) + positive-area check.
  - Byte-equal proof: `test_structure_layout_byte_equal_to_standalone_place` — each per-room `structure` layout.yaml == standalone `place` on the emitted room.yaml (all 4 rooms). PASS.
  - **Gate GREEN vs v0.42.0 baseline (666p/7s):** default **677 passed / 7 skipped** (+11 new), web **86 passed / 3 skipped**, ruff clean, mypy clean (57 files). NO version bump, NO commit (per task).
- [x] **executor: Phase S2 + Phase S3 — DONE 2026-06-17.**
  - Files TOUCHED (additive only): `roomestim/adapters/roomplan_structure.py` (extended: `_nearest_section` shared helper, `_assign_objects_to_sections`, `_assign_openings_to_sections`, `_room_from_section(objects=...)` + ADR 0037 guard, `parse_structure` wires openings/objects/doors/windows/degenerate-warning); `roomestim/cli.py` (`_add_structure_parser` +`--combined-gltf`/`--combined-usd`; `_cmd_structure` reuses shipped ADR 0049 `write_collection_gltf`/`write_collection_usd` + records `combined_ref`/`combined_usd_ref`); `README.md` (new honest `structure` section + known failure modes + CHANGELOG v0.43.0 row). **`_disclosure.py` NOT changed** (S1 already added `ROOMPLAN_STRUCTURE_SPLIT_NOTE`; reused). **Single-room code (`roomplan.py`/`model.py`/`collection.py`/existing exporters/readers/placement/room.yaml schema): ZERO touch** — room.yaml disclosure sidecar deliberately NOT emitted (would require editing shared `write_room_yaml`/schema → goldens break); CLI stderr note + ADR + README carry the disclosure (task-sanctioned).
  - Tests ADDED: `test_roomplan_structure_split.py` (+6: objects-per-policy, doors/windows wall_index-in-range, degenerate `UserWarning`, unidentified kept, equidistant tie-break, openings ingest; `test_provenance_measured_objects_empty_in_s1` updated→`test_provenance_measured` since objects now populated); `test_structure_cli.py` (+3: combined-gltf, no-combined-flag omits-ref, combined-usd skip-if-pxr).
  - **Actual S2 assignment on `capturedstructure_multiroom.json`:** 13 raw objects → **10 furniture KEPT** (table×4, storage×3, bed×2, sofa×1), 3 DROPPED (chair/sink/toilet per existing policy). Per room: bedroom=5, bedroom-2=3, bathroom=2, unidentified=0 furniture. **Doors 4 + windows 4 = 8** routed via `parentIdentifier`→parent-wall room: bedroom{door@4, window@0, window@2}, bedroom-2{door@5, window@0, window@4}, bathroom{door@2}, unidentified{door@0} — ALL re-based `wall_index` in-range for their room (ADR 0037 guard never trips). Total objects across 4 rooms = 18 (10+4+4).
  - **Combined export:** `--combined-gltf venue.glb` + `--combined-usd venue.usd` both write + record relative `combined_ref`/`combined_usd_ref` in manifest, reusing the shipped ADR 0049 writers (rooms at local origin, honest visual-assembly note printed — no inter-room pose). Verified end-to-end.
  - **Openings (S3):** `openings[]` merged into the wall pool (wall-like surfaces) → partition + wall surfaces + walls-only frame. Real fixture has 0 openings (additive no-op there); synthetic-opening test proves +1 wall-like surface (20→21).
  - **Degenerate (S3):** section with <3 assigned walls emits `UserWarning` (message = `ROOMPLAN_STRUCTURE_SPLIT_NOTE`, single source) + low-confidence minimal-box footprint; 0-wall section still yields a room (no crash). Real fixture min is bathroom=3 → NO warning on the real export. Tie-break = lowest section index (deterministic, strict `<`), tested.
  - **Byte-equal:** `roomplan.py`/`collection.py`/exporters/room.yaml schema untouched ⇒ all single-room + existing `collection` goldens byte-equal by construction; load-bearing `test_structure_layout_byte_equal_to_standalone_place` (S1) still PASS with objects now populated.
  - **GATE GREEN vs post-S1 (677p/7s):** default **686 passed / 7 skipped** (+9 new), web **86 passed / 3 skipped** (unchanged), ruff clean, mypy clean (57 files). NO version bump, NO commit (orchestrator handles release).
- [ ] code-review (separate lane) → verifier (full gate).
- [ ] version bump (MINOR/additive, target v0.43.0) + ADR 0050 → Accepted + memory update.
