...
- [ ] **(OQ-13e)** Live-mesh extraction (real A10a-substitute) — under what circumstances does v0.11+ add live-mesh corner extraction (alpha-shape / RANSAC / Hough on actual SoundCam PLY meshes) to replace the synthesised-shoebox revealed-tautology? — **Resolution candidate**: live extraction ships only when (i) the executor has SoundCam mesh download access; (ii) `floor_polygon_from_mesh` is augmented with non-convex polygon support per D6 deferred; (iii) corner err on at least 1 SoundCam room ≤ 10 cm against an authoritative GT (e.g., paper-published room dims acting as corner GT for axis-aligned rooms). Reverse if the v0.11+ executor confirms SoundCam meshes are not in fact convex-floored (which the v0.9 design risk register R-2 already flagged).
  - **Status update 2026-06-02 (v0.24.0, D82 — PARTIAL PROGRESS; stays OPEN):** condition (ii) is resolved — `floor_polygon_from_mesh` now implements opt-in concave-hull reconstruction (`shapely.concave_hull`, ADR 0042 §Status-update-v0.24.0 / PR1-equivalent), with the `MeshAdapter` default kept byte-equal convex. Conditions (i) and (iii) remain **UNMET**: SoundCam mesh download access is still unavailable, so no real-mesh corner-error ≤ 10 cm has been measured against an authoritative GT. The synthetic interior-vertex L-shape mesh generator (ADR 0042 §F item 1 / PR2) and the SoundCam-gated A10a non-tautological promotion (§F item 3 / PR4) did NOT land. The landed code documents the dense-cloud assumption honestly (sparse low-poly footprints undershoot area ~10–20 %); the ≤ 10 cm claim is therefore not yet empirically validated. OQ-13e stays **OPEN** pending SoundCam access (or another authoritative non-shoebox mesh GT).
...
> **PARTIALLY RESOLVED (2026-06-02, D83)** — VGGT-1B multi-view 스파이크(repo 밖 `/home/seung/mmhoa/spike-vggt-multiview/`,
> ARKitScenes raw Validation 10 방 48 view, genuine multi-view + metric GT). VERDICT = FALLBACK.
> **scale 하위질문 = RESOLVED(PASS)**: camera-baseline anchor 가 parallax 로부터 metric scale 을 복원,
> median scale error 1.6% / 10 방 중 6 방 ≤5% — single-pano 의 단일 cam_h single-point-of-failure 가
> 제거됨. **≤15 cm floor-geometry 정확도 하위질문 = OPEN(FALLBACK out-of-the-box)**: median corner error
> 22.4 cm(nv48), 2/10 방만 ≤15 cm, 8/10 ≤30 cm, median floor-area 43% undershoot(RoomPlan LiDAR ~8.5 cm 대비 ~2.6×).
> root cause 는 scale 이 아니라 sparse handheld sweep 의 **periphery under-coverage**(+ naive concave-hull
> front-end) — tractable front-end 문제이며 VGGT 의 scale·geometry 능력의 원리적 기각이 아니다. 정확도 하위질문은
> **OQ-59** 로 분기하여 추적한다. ADR 0045 §C / §Status-update-2026-06-02 참조.
...
>
> **부분 RESOLVED (2026-06-04, D85 / ADR 0046)** — **room-level 부분 해소**. `RoomModel` 에 room-level
> `provenance: Literal["measured","reconstructed","assumed"] = "assumed"` 구현(`model.py`). 정직한 least-claim
> 기본값 `"assumed"`; 실측 어댑터(roomplan/mesh/ace)만 `"measured"` 명시 단언, polycam 위임 상속 → image-derived/
> untagged 가 measured 로 둔갑하는 경로 0(독립 code-review §B CLOSED). YAML 은 0.2-draft 한정 방출(legacy 0.1 byte-equal),
> reader 키 부재 시 `"assumed"` 기본화; 스키마 additive(optional, root `additionalProperties:true`). 이로써 ADR 0045
> Reverse-criterion #4 / blocking gate #3 honesty 전제 충족 → image backend 출력 노출 차단 해제. **per-Surface
> provenance 는 OPEN(deferred)** — 3개 `additionalProperties:false` sub-object 편집 필요, 단일-출처 어댑터엔 불필요
> (YAGNI), follow-up. CLI/web "estimated" 라벨은 image adapter phase(빌드 플랜 P4)에서. Gates GREEN(default 320p/5s).
...
**OQ-59** — 집중된 floor-extraction front-end 스파이크: VGGT multi-view 스파이크(OQ-53, D83)가 scale 을
PASS 로 닫았으나 ≤15 cm floor-geometry 는 FALLBACK(median 22.4 cm)이고, 지배적 오차원은 sparse handheld
sweep 의 **periphery under-coverage**(median area 43% undershoot)와 naive concave-hull front-end 다 — scale
아님. 다음 lever 중 무엇이 ≤15 cm 게이트를 close 하는가: (a) raw concave hull 대신 RANSAC wall-plane corner
추출, (b) coverage-aware capture guidance(periphery 를 향한 sweep 계획), (c) multi-view TSDF depth fusion 후
footprint 추출, (d) fuller coverage 를 위한 VGGT-Omega 체크포인트. **multi-view 1급 경로(ADR 0045 §C)의
install-grade 승격 gate** — close 시 multi-view 가 first-class ≤15 cm 경로로 승격, 미달 시 rough-estimate tier
(HONEST metric scale + per-corner uncertainty)에 고정. ADR 0045 §C / §Status-update-2026-06-02 / D83 참조. Allocated 2026-06-02.

> **RESOLVED (2026-06-04, D84)** — front-end 레버 질문은 **NO**. 동일 캐시 nv48 VGGT cloud(10 방, 48 view) 위에서
> front-end 만 바꿔 동일 `best_fit_2d` metric 으로 재평가했다(Step-1 reproduction gate PASS, 10 방 전부 delta 0.00 cm).
> **PRIMARY 레버 (a) RANSAC wall-plane corner = 기각**: 단일 고정 파라미터에서 trivial convex hull 보다 나쁘다
> (median 19.30 cm vs convex 17.13 cm, 2/10 vs 4/10 ≤15 cm) + high-variance(41069048 3.4 cm win / 41159519 105 cm
> blow-up). **최고 배포가능 = `convex_band` 17.13 cm / 4-of-10 도 여전히 ≤15 cm FAIL**(baseline 22.41 cm / 2-of-10
> 대비 −5 cm·+2 방 무비용 개선이나 install-grade 미달). **ORACLE `sweep_best` 11.60 cm / 7-of-10** 은 headroom 을
> 증명하나 per-room GT-tuned → 배포 불가(method 아닌 ceiling). **root cause = corner-fitting 아니라 periphery
> under-coverage**(OQ-53 재확인). 따라서 multi-view 를 first-class ≤15 cm 경로로 **승격하지 않고 rough-estimate
> tier(HONEST metric scale + per-corner uncertainty)에 고정**한다; `convex_band` 는 rough-tier 내 무비용 in-tier
> upgrade(median win 이지 per-room dominance 아님 — 3/10 방 regress)이지 게이트 승급 아님. 잔여 레버 (b) coverage-aware
> capture / (c) TSDF fusion / (d) VGGT-Omega 는 모두 *coverage* 를 공략하며 front-end-only 변경이 아니므로 각각
> 별도 스파이크로 defer. RANSAC OOM(OQ-53 의 10/10 크래시)은 repo 밖 `frontends.py` 에서 수정(SVD `full_matrices=False`
> + ≤100 k seeded subsample) — 위 RANSAC 수치는 진짜 데이터. ADR 0045 §Status-update-2026-06-04 / §C / D84,
> 스파이크 아티팩트 `/home/seung/mmhoa/spike-vggt-multiview/`(`OQ59_VERDICT.md`, `out/oq59_verdict.json`) 참조.

**OQ-60** — near-horizon 가드: 절대 반경 상한(`_MAX_PLAUSIBLE_RADIUS_M`, D89/v0.25.2) 대신 상대(relative)
outlier 테스트(`max_radius > k × median(other radii)`)를 쓰면 "큰 방"과 "코너 오검출"을 더 깨끗이 분리할 수 있는가?
가설: 진짜 mis-detection 은 한 코너 반경만 ≫ 나머지(ratio 큼), legit 큰 방은 모든 코너가 비례해 큼(ratio 작음).
경쟁 가설: 코너에 가까이 둔 카메라(off-center)도 극단 ratio 를 만들어 상대 테스트가 정상 방을 false-reject 할 수 있다.
ADR 0045 §Status-update-2026-06-05c / D89. Allocated 2026-06-05.

> **RESOLVED (2026-06-06, D90) — 상대 테스트 기각, 절대 상한 20 m 유지(코드 변경 0).** 240 실파노(seed=7, 주거 120 +
> 사무 120) 실측: **예측 코너반경 ratio 최대 = 1.84**(GT ratio 최대 1.59). 후보 k∈{4,6,8,10,15} 전부 0/240 거부 →
> 상대 테스트는 **구조적으로 무력**하다(HorizonNet force_cuboid 후처리가 네 코너를 비례 이동시켜 ratio 를 낮게 유지;
> "한 코너만 ratio 12" 분리 신호는 cuboid 출력에선 발생 안 함). 경쟁 가설(off-center→고 ratio)도 데이터로는 미발현
> (직사각형 방에선 대각 코너가 함께 스케일되어 ratio 상한 ~2.2). **절대 상한도 완벽 분리는 불가**: 명백한 artifact
> (pred 28.1 m, GT 3.6 m)와 legit 대형 오피스(pred 27.9 m, GT 47.4 m)가 0.2 m 차로 겹친다 → 어떤 절대 임계도
> 둘을 robust 분리 못 함. 그러나 20 m 의 거부 4건은 전부 정당: 2건 = 진짜 환각(GT 3.6/7.4 m), 2건 = >~40 m 초대형
> 방으로 rough tier 범위 밖(단일 파노 재구성 불가, 에러 메시지가 depth 백엔드 안내). scientist 의 "40 m 로 상향" 권고는
> **내부모순**(40 m 는 28.1 m artifact #1 을 통과시켜 3.6 m 방을 28 m 로 방출 → 가드 무력화)이라 기각. ∴ 현 구현이
> 최선. ADR 0045 §Status-update-2026-06-06 / D90 / scientist 분석(240 panos, ratio dist + 2×2 분리표) 참조.
...
> **multi-view(VGGT) 스파이크 후속 (2026-06-02, D83)**: blocking gate #2(OQ-53) multi-view 스파이크 수행 →
> VERDICT FALLBACK. scale 하위질문 PASS(camera-baseline anchor, median scale error 1.6%, 6/10 ≤5% — single-pano
> cam_h single-point-of-failure 제거), ≤15 cm floor-geometry 는 OPEN(median 22.4 cm, periphery under-coverage
> root cause). multi-view = *조건부* 1급 경로(install-grade ship 전 OQ-59 선행). OQ-53 부분 RESOLVED, OQ-59 신규.
> ADR 0045 header 는 PROPOSED 유지(gate #2 정확도 절반 미충족 + OQ-52/OQ-54 미해소).

> **floor-extraction front-end 스파이크 (2026-06-04, D84)**: OQ-53/D83 이 지목한 front-end 레버를 동일 캐시 nv48 cloud
> 위에서 검증 → **VERDICT FALLBACK(hardened), OQ-59 RESOLVED(NO)**. 배포가능 front-end 그 어느 것도 ≤15 cm 를 close 하지
> 못했다 — PRIMARY RANSAC wall-plane corner 기각(median 19.30 cm vs convex 17.13 cm, 2/10 vs 4/10, high-variance),
> 최고 배포가능 `convex_band` 17.13 cm / 4-of-10 도 FAIL, oracle `sweep_best` 11.60 cm / 7-of-10 은 headroom 증명이나
> GT-tuned·배포 불가. root cause = periphery under-coverage(corner-fitting 아님). multi-view = first-class ≤15 cm 승격
...
## 4-Candidate Follow-ups (A/B/C/D) - 2026-06-08
- [ ] (A) spatial_engine engine-schema default — confirm fail-loud (no machine-specific default) is acceptable vs a bundled fallback, given the "schema read at write time, never copied" contract (layout_yaml.py docstring). — Decides A's resolution strategy; affects whether engine validation silently no-ops anywhere.
...
## CapturedStructure splitter (ADR 0050) - 2026-06-17
