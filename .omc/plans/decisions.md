...
## D104 — Ambisonics 리그 좌표 생성 SHIPPED experimental (ADR 0041 PR2+PR3; v0.39.0; CAND-3, 2026-06-17)

**맥락**: ADR 0041 PR1(`x_target_algorithm` round-trip, D102)만 출하돼 있었고 PR2-4(ambisonics
producer)는 DEFER 상태였다. §D-3a 는 두 포인트로 나뉜다 — point 1(engine 이 리그를 SH 디코더로
식별·라우팅)은 END-TO-END 게이트로 여전히 UNMET(`grep -ni ambison spatial_engine/require.md` → 0 hits,
engine-team 합의 없음), point 2 는 end-to-end 디코딩 불확실성을 고지하는 한 roomestim 측 **좌표 생성**을
명시적으로 허용한다.

**결정**: PR2(platonic 리그 기하) + PR3(dispatch/CLI 배선)를 **EXPERIMENTAL·opt-in·정직고지** 좌표-생성
슬라이스로 출하한다. PR4(t-design)는 DEFER 유지. 신규 `roomestim/place/ambisonics.py`
`place_ambisonics(order)`: order→리그 closed-form unit 정점(1=octahedron6/2=icosahedron12/
3=dodecahedron20, 각 n≥(N+1)²), `radius_m` 스케일, aim=리스너 지향, `regularity_hint=IRREGULAR`,
`target_algorithm=AMBISONICS`. **numpy-only, 신규 의존 0**. `--algorithm ambisonics --order {1,2,3}`
를 `place`/`run` 양쪽에 추가, `dispatch.run_placement(order=...)` 분기(order 누락 시 `--order` 안내
ValueError).

**정직성(load-bearing)**: 단일 진실원천 상수 `AMBISONICS_RIG_DISCLOSURE` — roomestim 은 리그 좌표만
방출하며 SH 인코딩/디코딩·decode matrix·decoder-type 선택은 engine 책임이고 end-to-end 라우팅 계약(§D-3a
point 1)은 미확정(UNCONFIRMED)임을 명시. 모든 ambisonics `place`/`run` 호출에서 stderr 로 NOTE 가 항상
출력(불가피); `--el-deg`/`--n-speakers` 가 비기본값이면 무시 경고(WARN, NOT silent). 가짜 능력 주장 0.

**검증**: `tests/test_ambisonics_placement.py`(+26 tests) — 각도 정밀도(≤5°, 실제 <1e-6 rad), 대칭(등반경
+ centroid≈0), 준-등방성 PROXY(numpy 2차모멘트 M=VᵀV/n=(1/3)I, cond<1.01; scipy `sph_harm`→`sph_harm_y`
1.15+ 리네임 회피), n≥(N+1)², round-trip 라벨 보존(PR1) + write→read→write 고정점, R10/R11 write
pre-flight, 에러 계약(order∉{1,2,3}, dispatch order=None), CLI WARN+NOTE. 신규 golden
`tests/fixtures/golden/place_ambisonics_order1_octa.yaml`(validate=False path, x_target_algorithm:
AMBISONICS·IRREGULAR·6 spk·D56 9dp). VBAP/DBAP/WFS golden **byte-equal**(ambisonics 는 algorithm==
ambisonics 일 때만 방출). MINOR 범프(0.38.0→0.39.0; 신규 실험 능력).
**변경 파일**: `roomestim/place/ambisonics.py`(NEW), `roomestim/place/__init__.py`,
`roomestim/place/dispatch.py`, `roomestim/cli.py`, `tests/test_ambisonics_placement.py`(NEW),
`tests/fixtures/golden/place_ambisonics_order1_octa.yaml`(NEW), `README.md`,
`docs/adr/0041-ambisonics-placement-design.md`(Status + Status-update 2026-06-17),
`roomestim/__init__.py`·`pyproject.toml`(0.39.0), 본 D104.
**Cross-refs**: ADR 0041 §D-3a(point 1 게이트=experimental 제거 트리거), D102(PR1), `_disclosure.py`
`POLYGON_ISM_GEOMETRY_NOTE`(고지 상수 패턴 미러).
