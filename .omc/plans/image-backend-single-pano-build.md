...
- [x] **P3** `adapters/image.py` 단일-파노 — **완료(2026-06-04)**. torch-free 지오메트리 코어 `_corners_to_room`(metric_layout 삼각법: `r=cam_h/tan(-v_floor)`, x=r·sin u, z=-r·cos u, ceil=cam_h+r·tan v_ceil) + torch path `_infer_corners`(lazy import, vendored HorizonNet + ckpt resolve). `ImageAdapter(CaptureAdapter)` — cam_h from `ScaleAnchor("known_distance",len_m)` else default+warn. material UNKNOWN(§E)/provenance=reconstructed/objects=[], `walls_from_floor_polygon`+`default_listener_area` 재사용. honesty 경고 3종(experimental rough≠install-grade / Manhattan pano / scale-source OQ-58). **end-to-end smoke(spike venv): roomA.png→parse→RoomModel→write/read yaml→VBAP(6)→layout.yaml OK** — ingest→place→export 이미지-파생 지오메트리에서 동작. 정확도 rough(out-of-domain 예상, 라벨링됨). 경계 torch-free 유지. executor→code-reviewer APPROVE(trig 오라클 대조 검증·0 blocker)→minor 2(절대코너 assert, stale comment) 적용. Gates: default 332p/5s, web 86p/4s, ruff/mypy(47)/tense EXIT0. 미커밋(이 P3 커밋 예정).
- [x] **P4** CLI 노출 + ESTIMATED 라벨링 — **완료(2026-06-04)**. `image` 백엔드를 CLI ingest+run에 배선(`--cam-height` scale anchor, `--weights` st3d/zind, `--accept-zind-tou`, `--experimental` 하드 게이트). 게이트는 torch-free(adapter import 전 `_ExperimentalGate` raise → exit 1). provenance=reconstructed일 때만 ESTIMATED 고지(rough≠install-grade, ≤15cm는 LiDAR/RoomPlan 한정) — measured 백엔드 무영향. **end-to-end CLI smoke(spike venv)**: `roomestim run --backend image --experimental --cam-height 1.6 --input roomA.png --algorithm vbap --n-speakers 6` → room.yaml(provenance reconstructed)+layout.yaml, ESTIMATED 고지, exit0. executor→code-reviewer APPROVE(4 invariant 런타임 검증, 0 blocker). 13 신규 테스트(torch-free). Gates: default 345p/5s, web 86p/4s, ruff/mypy(47)/tense EXIT0. **web 노출 + 실제 per-corner uncertainty(OQ-57)는 정직하게 defer**(OQ-57 미해결 calibration; 가짜 숫자 금지). 미커밋(이 P4 커밋 예정).
- [x] **P5** 출하 wrap — **진행(2026-06-04)**. v0.24.0→**0.25.0 MINOR**(pyproject+__init__), `RELEASE_NOTES_v0.25.0.md`(신규), README `--backend image` 문서, ADR 0045 §Status-update-2026-06-04b(+footer 정정: rough tier 구현·출하 기록, header PROPOSED 유지), D86. 정직성 전반: reconstructed/UNKNOWN/cam-height scale/rough 정확도/ESTIMATED 고지/≤15cm는 LiDAR 한정. 최종 full gate + 독립 verifier 후 커밋(이 단계).
